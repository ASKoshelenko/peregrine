export const pains = [
  ...
  { href: "#fleet", en: ["A heterogeneous fleet", "Runtime and delegate updates change what executes where.", "Keep a target matrix with separate quality, latency and size budgets.", "Compare the device lanes", "What re-validates models when a runtime changes?"], uk: ["Неоднорідний fleet", "Оновлення runtime і delegate змінюють, що й де виконується.", "Тримайте матрицю цілей з окремими бюджетами якості, latency й розміру.", "Порівняти контури пристроїв", "Що повторно валідує моделі після зміни runtime?"] },
  { href: "#trace", en: ["The dataset keeps moving", "Relabels break comparison even when no new image appears.", "Version snapshots, label space and calibration membership together.", "Inspect the dataset fingerprint", "Does a label-only change force a re-baseline today?"], uk: ["Дані постійно рухаються", "Relabeling ламає порівняння навіть без нових зображень.", "Версіонуйте snapshot, простір міток і склад калібрування разом.", "Перевірити fingerprint даних", "Чи змушує зміна самих лише міток робити re-baseline?"] },
  { href: "#trace", en: ["January cannot be reproduced", "Configs become folklore and dashboards drift.", "Materialize the full config and bind it to model, dataset and environment hashes.", "Open the evidence chain", "Could January's run be rebuilt from its config alone?"], uk: ["Січень не відтворити", "Конфіги стають фольклором, а dashboards дрейфують.", "Матеріалізуйте повний config і звʼязуйте його з хешами моделі, даних та середовища.", "Відкрити ланцюжок доказів", "Чи можна відбудувати січневий run лише з його config?"] },
  { href: "#trace", en: ["Queues, quotas and the bill", "Compute availability fails exactly when a release needs proof.", "Register the question and cost bound before allocating a run; retain failed attempts.", "Open the retained run evidence", "Who owns the training bill—and has quota blocked a deadline?"], uk: ["Черги, квоти й рахунок", "Compute availability підводить саме тоді, коли релізу потрібен доказ.", "Зафіксуйте питання й cost bound до виділення ресурсів; зберігайте невдалі спроби.", "Відкрити збережені докази запуску", "Хто відповідає за training bill — і чи блокувала квота дедлайн?"] },
];
